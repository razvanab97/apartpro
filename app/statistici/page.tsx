...
const BOOKING_COMISION = 0.17

function _fmtRon(v: number) { return v.toLocaleString('ro-RO', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }
function _fmtDate(iso: string) { return new Date(iso + 'T12:00:00').toLocaleDateString('ro-RO', { day: 'numeric', month: 'short', year: 'numeric' }) }
function _fmtDateShort(iso: string) { return new Date(iso + 'T12:00:00').toLocaleDateString('ro-RO', { day: 'numeric', month: 'short' }) }

function _getWeekPeriod(offsetWeeks = 0): { from: string; to: string; payDate: string } {
  const now = new Date()
  const day = now.getDay()
  const diffToFriday = (day + 2) % 7
  const friday = new Date(now); friday.setDate(now.getDate() - diffToFriday + offsetWeeks * 7); friday.setHours(0,0,0,0)
  const thursday = new Date(friday); thursday.setDate(friday.getDate() + 6)
  const payFriday = new Date(friday); payFriday.setDate(friday.getDate() + 7)
...
type BkPerioada = {
  ...
  rezervari: BkRez[]; total_brut: number; total_comision: number
  total_net: number; status: string; id?: string
}

function BookingRapoarteContent() {
  ...
  const { toast: bkToast, show: bkShow } = useToast()
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px 40px' }}>
        ...
        <div style={{ flex: 1, textAlign: 'center' as const }}>
          {loadingBk ? <span style={{ color: 'rgba(159,215,255,0.4)' }}>Se încarcă...</span> : perioada && <>
            ...
            <div style={{ fontSize: 11, color: 'rgba(159,215,255,0.4)', marginTop: 2 }}>
              ...
              {' · '}Plată estimată: <span style={{ color: '#FCD34D' }}>{_fmtDateShort(perioada.payDate)}</span>
            </div>
          </>}
        </div>
        <button onClick={() => setWeekOffset(w => w + 1)} style={{ width: 36, height: 36, borderRadius: 8, border: '0.5px solid rgba(159,215,255,0.2)', background: 'rgba(159,215,255,0.06)', color: 'rgba(214,228,244,0.8)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {!loadingBk && perioada && (
        <div style={{ ...panel, borderTop: `2px solid ${perioada.status==='incasat'?'#4ADE80':'#0055A5'}` }}>
          ...
          <div style={{ padding: '12px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            {[
              { label: 'Sumă brută', value: _fmtRon(perioada.total_brut), color: 'rgba(214,228,244,0.7)' },
              { label: `Comision Booking (${Math.round(BOOKING_COMISION*100)}%)`, value: '- ' + _fmtRon(perioada.total_comision), color: '#F87171' },
              { label: 'NET estimat', value: _fmtRon(perioada.total_net), color: '#4ADE80' },
            ].map(item => (
      ...
      {!loadingBk && (perioada?.rezervari.length||0) > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' as const }}>
          <select value={filter.apt} onChange={e => setFilter(f => ({ ...f, apt: e.target.value }))}
            ...
            {bkApts.map(a => <option key={a.id} value={a.nota}>{a.nota} · {a.nume}</option>)}
          </select>
      ...
      {!loadingBk && filteredRez.length === 0 && (
      ...
      {!loadingBk && filteredRez.map((r, idx) => {
          ...
          <div key={r.id||idx} style={{ ...panel, borderLeft: `3px solid ${sb.color}40` }}>
            <div style={{ padding: '10px 14px' }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const, marginBottom: 6 }}>
                <span style={{ fontSize: 11, color: 'rgba(159,215,255,0.45)' }}>CI: {_fmtDateShort(r.data_checkin)}</span>
                <span style={{ fontSize: 11, color: 'rgba(159,215,255,0.25)' }}>→</span>
                <span style={{ fontSize: 11, color: 'rgba(159,215,255,0.45)' }}>CO: {_fmtDateShort(r.data_checkout)}</span>
                {r.nr_nopti && <span style={{ fontSize: 11, color: 'rgba(159,215,255,0.35)' }}>· {r.nr_nopti}n</span>}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
                {[
                  { label:'BRUT', v:_fmtRon(Number(r.valoare_bruta||r.suma_incasata||0)), c:'rgba(214,228,244,0.7)' },
                  { label:'COMISION', v:'- '+_fmtRon(r._comision), c:'#F87171' },
                  { label:'NET', v:_fmtRon(r._net), c:'#4ADE80' },
                ].map(x => (
                  <div key={x.label} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '6px 8px' }}>
                    <div style={{ fontSize: 9, color: 'rgba(159,215,255,0.3)', marginBottom: 2 }}>{x.label}</div>
                    <div style={{ fontSize: 13, fontWeight: x.label==='NET'?700:600, color: x.c, fontFamily: 'monospace' }}>{x.v}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {istoric.map((p, idx) => {
            ...
            return (
              <div key={idx} onClick={() => setWeekOffset(idx * -1 - 1)}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, fontWeight: 500, color: 'rgba(214,228,244,0.7)' }}>{_fmtDateShort(p.from)} — {_fmtDateShort(p.to)}</div>
                  <div style={{ fontSize: 11, color: 'rgba(159,215,255,0.35)', marginTop: 2 }}>{p.rezervari.length} rez · Plată: {_fmtDateShort(p.payDate)}</div>
                </div>
                <div style={{ textAlign: 'right' as const }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: '#4ADE80', fontFamily: 'monospace' }}>{_fmtRon(p.total_net)}</div>
...
export default function StatisticiPage() {
  ...
  return (
    <>
      ...
      <div style={S.page}>
        <div style={S.tabs}>
          {([['dashboard','📊 Dashboard'],['evolutie','📈 Evoluție'],['upload','📤 Upload'],['booking','💰 Booking $']] as [Tab,string][]).map(([t,l]) => (
        ...
        {tab === 'booking' && <BookingRapoarteContent/>}
      </div>
      <Toast toast={toast} />
    </>
  )
