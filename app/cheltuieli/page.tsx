...
// Date din poza Excel — nota => { col: valoare }
const DEFAULTS: Record<string, Record<string, number>> = {
  ...
  'N33':    { chirie: 3080 },
  'Gara':   { chirie: 2250, internet: 70 },
  'Podu Ros': { chirie: 2500 },
  'L83':    { chirie: 2000 },
...
export default function CheltuieliPage() {
  ...
  // Seed valorile din poza pentru luna curentă dacă nu există
  async function seedDefaults() {
    setSeeding(true)
    const inserts: any[] = []
    apts.forEach(apt => {
      const key = apt.nota || apt.nume
      const defs = DEFAULTS[key]
      if (!defs) return
      COLS.forEach(col => {
        const val = defs[col.key]
        if (val == null || val === 0) return
        if (rows[apt.id]?.[col.key]) return // deja există
        inserts.push({
    ...
    if (inserts.length === 0) { show('info', 'Toate valorile sunt deja completate'); setSeeding(false); return }
    const { error } = await supabase.from('cheltuieli').insert(inserts)
    if (error) { show('error', error.message) } else {
      show('success', `${inserts.length} valori importate din template`)
      await load()
    }
    setSeeding(false)
  }
  ...
  const colTotal = (col:string) => apts.reduce((s,a) => s+Number(rows[a.id]?.[col]?.valoare||0),0)
  ...
  function renderRow(apt: any, isLast: boolean) {
    ...
    return (
      <div key={apt.id} className="ch-row" style={{
        display: 'grid',
        gridTemplateColumns: `180px repeat(${COLS.length}, 1fr) 150px`,
        borderBottom: isLast ? 'none' : '1px solid rgba(159,215,255,0.06)',
      }}>
        {/* Nome */}
        <div style={{ ...td, gap:7, borderRight:'1px solid rgba(159,215,255,0.06)' }}>
          {apt.nota
              ...
              {isEdit ? (
                <div style={{ display:'flex', alignItems:'center', gap:4, width:'100%' }}>
                  <input
                    ...
                    min={0}
                  />
                  <button onClick={commitEdit} disabled={saving} style={{ background:'none', border:'none', cursor:'pointer', color:'#4ADE80', padding:2, display:'flex', flexShrink:0 }}>
                    <Check size={13}/>
                  </button>
                  <button onClick={() => {setEditing(null);setEditVal('')}} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(159,215,255,0.4)', padding:2, display:'flex', flexShrink:0 }}>
                    <X size={13}/>
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <div className="cell-actions" style={{ display:'flex', gap:2 }}>
                    <button
                      title="Editează"
                      onClick={() => startEdit(apt.id, col.key)}
                      style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(159,215,255,0.35)', padding:'3px 4px', display:'flex', borderRadius:4 }}>
                      <Pencil size={11}/>
                    </button>
                    <button
                      title={isPaid ? 'Marchează neplătit' : 'Marchează plătit'}
                      onClick={() => togglePaid(apt.id, col.key)}
      ...
      <div style={{ marginBottom:28 }}>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:10 }}>
          ...
          {secTotal > 0 && <>
            ...
            <span style={{ fontSize:11, color:'rgba(159,215,255,0.5)' }}>{secTotal.toLocaleString('ro-RO')} RON total</span>
            {secPaid > 0 && <>
              <span style={{ fontSize:11, color:'rgba(159,215,255,0.25)' }}>·</span>
              <span style={{ fontSize:11, color:'#4ADE80' }}>{secPaid.toLocaleString('ro-RO')} RON plătit</span>
            </>}
          </>}
        </div>
        <div style={{ background:'rgba(214,228,244,0.03)', border:'1px solid rgba(159,215,255,0.09)', borderRadius:14, overflow:'hidden' }}>
          {/* Header */}
          <div style={{ display:'grid', gridTemplateColumns:`180px repeat(${COLS.length}, 1fr) 150px`, background:'rgba(11,18,32,0.5)', borderBottom:'1px solid rgba(159,215,255,0.09)' }}>
            <div style={th}>Apartament</div>
            {COLS.map(c => (
              <div key={c.key} style={th}>
                {c.label}
                <span style={{ fontSize:10, color:'rgba(159,215,255,0.25)', marginLeft:5 }}>/{c.due}</span>
              </div>
            ))}
          ...
          {/* Footer */}
          <div style={{ display:'grid', gridTemplateColumns:`180px repeat(${COLS.length}, 1fr) 150px`, borderTop:'1px solid rgba(159,215,255,0.09)', background:'rgba(11,18,32,0.3)' }}>
            <div style={{ ...th, color:'rgba(159,215,255,0.25)', fontSize:10 }}>Total</div>
            {COLS.map(c => {
              const t = colTotal(c.key)
              return (
                <div key={c.key} style={{ ...th, color: t>0?'rgba(77,163,255,0.7)':'rgba(159,215,255,0.12)', fontWeight:500, fontSize:12 }}>
                  {t > 0 ? t.toLocaleString('ro-RO') : '—'}
                </div>
              )
            })}
            <div style={{ ...th, borderRight:'none', color:'rgba(77,163,255,0.9)', fontSize:13, fontWeight:500 }}>
  ...
  return (
    <>
        ...
        actions={
          <button
            ...
            style={{
              ...
              background:'rgba(77,163,255,0.08)', color:'var(--accent-blue)',
              cursor:'pointer', transition:'all .15s',
              opacity: seeding ? 0.6 : 1,
            }}
          >
            <Plus size={13}/>
            {seeding ? 'Se importă...' : 'Import valori fixe'}
          </button>
        }
      />
      <style>{`
      ...
      `}</style>
      <div style={{ padding:'0 24px 32px', overflowX:'auto', minWidth:0 }}>
        {loading
