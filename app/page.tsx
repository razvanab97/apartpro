  ...
  return(
    <>
      ...
      <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:'14px 16px 60px',display:'flex',flexDirection:'column',gap:12}}>
        ...
        {/* ══ CURĂȚENIE ASTĂZI ══ */}
        <div style={{background:'rgba(239,159,39,0.15)',border:'2px solid #EF9F27',borderRadius:10,padding:'12px 16px',marginBottom:4}}>
          <div style={{color:'#FCD34D',fontWeight:700,fontSize:13,marginBottom:8}}>🧹 Curățenie astăzi — CO: {coAziCur.length} | CI: {ciAziCur.length}</div>
          {coAziCur.map((r:any,i:number)=>(
            <div key={i} style={{color:'#F87171',fontSize:12,marginBottom:4}}>
              CO: {r.apartament?.nota} {r.apartament?.nume} — {r.nume_client}
            </div>
          ))}
          {ciAziCur.map((r:any,i:number)=>(
            <div key={i} style={{color:'#4ADE80',fontSize:12,marginBottom:4}}>
              CI: {r.apartament?.nota} {r.apartament?.nume} — {r.nume_client} ({r.nr_persoane||1} pers)
            </div>
          ))}
          {coAziCur.length===0&&ciAziCur.length===0&&<div style={{color:'rgba(255,255,255,0.4)',fontSize:12}}>State gol — datele nu ajung</div>}
        </div>
