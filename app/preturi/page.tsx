...
export default function PreturiPage() {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/booking-scan',{
        method:'POST', headers:{'Content-Type':'application/json'},
        body:JSON.stringify({checkin:checkinMonitor,checkout:checkoutMonitor})
      })
      const data = await res.json()
      if(!res.ok||data.error) throw new Error(data.error||`Eroare ${res.status}`)
      setScan({
        status:'done', results:data.results||[], total:data.total,
        lowestPrice:data.lowestPrice, weAreLowest:data.weAreLowest,
        ourLowestRank:data.ourLowestRank, checkin:checkinMonitor, checkout:checkoutMonitor,
          ...
          {apts.map((apt,i)=>(
            <div key={apt.id} style={{padding:'10px 16px',
              ...
              <div style={{display:'flex',alignItems:'center',gap:6,minWidth:60}}>
                ...
                <span style={{fontSize:10,lineHeight:1}}>{ocupate.has(apt.id)?'🔴':'🟢'}</span>
              </div>
        ...
        <div style={{...panel,border:'1px solid rgba(99,179,237,0.2)',background:'rgba(15,30,55,0.6)'}}>
          ...
          <div style={{padding:'12px 16px',borderBottom:'1px solid rgba(99,179,237,0.12)',
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              ...
              <button onClick={()=>{setShowHistory(!showHistory);if(!showHistory)loadHistory()}} style={{
                padding:'4px 10px',borderRadius:6,fontSize:11,cursor:'pointer',
                border:'1px solid rgba(99,179,237,0.2)',background:showHistory?'rgba(99,179,237,0.12)':'transparent',
          ...
          {scan.status==='error'&&(
            <div style={{padding:'14px 16px',display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:'rgba(248,113,113,0.5)',marginTop:2}}>Încearcă din nou sau verifică conexiunea.</div>
              </div>
          ...
          {scan.status==='idle'&&(
            <div style={{padding:'20px 16px',textAlign:'center' as const,color:'rgba(147,197,253,0.3)',fontSize:12}}>
              Apasă "Caută pe Booking" — extrage automat top 5 din Iași sortate după preț + număr total proprietăți
            </div>
          )}
            ...
            <div style={{borderTop:'1px solid rgba(99,179,237,0.12)'}}>
              ...
              {loadingHistory?(
                ...
                <div style={{overflowX:'auto' as const}}>
                  {/* Header tabel */}
                  <div style={{
                    display:'grid',gridTemplateColumns:'90px 110px 55px 65px 55px 1fr',
                    padding:'6px 16px',gap:8,
                    fontSize:10,color:'rgba(147,197,253,0.35)',textTransform:'uppercase' as const,letterSpacing:'.05em',
                    borderBottom:'1px solid rgba(99,179,237,0.08)',
                  }}>
                    <span>Data/Oră</span><span>Perioadă</span><span>Total</span><span>Minim</span><span>Noi</span><span>Top 1</span>
                  </div>
                  {history.map((h,i)=>{
                    const top1 = h.top5?.[0]
                    return (
                      <div key={h.id} style={{
                        ...
                        <span style={{fontSize:11,fontFamily:'monospace',color:'rgba(214,228,244,0.6)'}}>
                          {fmtDateTime(h.scanned_at)}
                        </span>
                        <span style={{fontSize:11,color:'rgba(147,197,253,0.6)'}}>
                          {h.checkin?.slice(5)} → {h.checkout?.slice(5)}
                        </span>
                        <span style={{fontSize:11,fontFamily:'monospace',
                          color:h.total_properties?'rgba(214,228,244,0.8)':'rgba(147,197,253,0.3)'}}>
                          {h.total_properties??'—'}
                        </span>
                        <span style={{fontSize:12,fontFamily:'monospace',fontWeight:600,
                          color:h.we_are_lowest?'#4ADE80':'rgba(214,228,244,0.8)'}}>
                        ...
                        <span style={{fontSize:11,color:'rgba(214,228,244,0.55)',
                          overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' as const}}>
