...
import {
  ...
  Receipt, FileText, Settings, CheckSquare, TrendingUp, Upload, RefreshCw, CalendarDays
} from 'lucide-react'
...
const navItems = [
  ...
  { href: '/calendar', icon: CalendarDays, label: 'Calendar', group: 'P' },
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <aside style={{
        width: 188, flexShrink: 0, display: 'flex', flexDirection: 'column', height: '100vh',
        background: 'rgba(11,18,32,0.85)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)',
        borderRight: '1px solid rgba(159,215,255,0.08)', position: 'relative',
      }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '1px', background: 'linear-gradient(90deg,transparent,rgba(159,215,255,0.3),transparent)' }}/>
        <div style={{ padding: '16px 14px 13px', borderBottom: '1px solid rgba(159,215,255,0.07)', display: 'flex', alignItems: 'center', gap: 9 }}>
        ...
        <nav style={{ flex: 1, overflowY: 'auto', padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {groups.map(g => {
            ...
            return (
              <div key={g.key}>
                <div style={{ fontSize: 9, fontWeight: 600, color: 'rgba(159,215,255,0.25)', textTransform: 'uppercase', letterSpacing: '0.9px', padding: '0 8px', marginBottom: 3 }}>{g.label}</div>
                  ...
                  {items.map(({ href, icon: Icon, label }) => {
                    ...
                    return (
                      <Link key={href} href={href} style={{
                        ...
                        {label}
                        {active && <div style={{ marginLeft: 'auto', width: 5, height: 5, borderRadius: '50%', background: '#4DA3FF', boxShadow: '0 0 6px rgba(77,163,255,0.9)' }} />}
                      </Link>
                    )
                  })}
                </div>
...
export function PageHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: ReactNode }) {
  return (
    <div style={{
      ...
      padding: '11px 20px',
      background: 'rgba(14,27,43,0.6)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)',
